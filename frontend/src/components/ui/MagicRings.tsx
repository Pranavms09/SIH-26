...
void main() {
  ...
  c += (n - 0.5) * uNoiseAmount * min(coverage * 3.0, 1.0);
...
export default function MagicRings({
  ...
  const propsRef = useRef<Required<Omit<MagicRingsProps, 'className' | 'style'>> | null>(null);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      hoverAmountRef.current += (targetHover - hoverAmountRef.current) * 0.05;
    ...
    const tryStart = () => {
      if (isVisible && isPageVisible && frameId === 0) {
        startTime = performance.now() - (uniforms.uTime.value / (propsRef.current?.speed || 1)) * 1000;
        frameId = requestAnimationFrame(animate);
      }
    };
    const tryStop = () => {
